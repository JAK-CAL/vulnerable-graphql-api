import {AttackGene, OperationCatalogEntry, SecurityTestConfig, SequenceStep} from './types';
...
function authStep(actor: string, config: SecurityTestConfig): SequenceStep {
    return {
        actor: actor,
        operationName: config.auth.meOperationName,
        query: config.auth.meQuery,
        variables: {},
        purpose: 'AUTH(' + actor + '): establish or verify actor session'
    };
}

export function lowerGeneToSequence(gene: AttackGene, catalog: OperationCatalogEntry[], config: SecurityTestConfig): SequenceStep[] {
    if (gene.type === 'BOLA_READ') {
        return lowerBolaRead(gene, catalog, config);
    }
    if (gene.type === 'BOLA_UPDATE_DELETE') {
        return lowerBolaUpdateDelete(gene, catalog, config);
    }
    if (gene.type === 'STALE_OBJECT_ACCESS') {
        return lowerStaleObjectAccess(gene, catalog, config);
    }
    if (gene.type === 'BFLA_ADMIN_LIKE_OP') {
        return lowerBfla(gene, catalog, config);
    }
    if (gene.type === 'BOPLA_SENSITIVE_FIELD_READ') {
        return lowerBopla(gene, catalog, config);
    }
    return [];
}

function lowerBolaRead(gene: AttackGene, catalog: OperationCatalogEntry[], config: SecurityTestConfig): SequenceStep[] {
...
function lowerBfla(gene: AttackGene, catalog: OperationCatalogEntry[], config: SecurityTestConfig): SequenceStep[] {
...
function lowerBolaUpdateDelete(gene: AttackGene, catalog: OperationCatalogEntry[], config: SecurityTestConfig): SequenceStep[] {
    const createOp = findOperation(catalog, gene.setupResolver);
...
function lowerStaleObjectAccess(gene: AttackGene, catalog: OperationCatalogEntry[], config: SecurityTestConfig): SequenceStep[] {
    const createOp = findOperation(catalog, gene.setupResolver);
...
function lowerBopla(gene: AttackGene, catalog: OperationCatalogEntry[], config: SecurityTestConfig): SequenceStep[] {
    ...
    if (op.name === 'passwordReset') {
        return lowerPasswordResetBopla(gene, catalog, op, config);
    }
...
function lowerPasswordResetBopla(gene: AttackGene, catalog: OperationCatalogEntry[], resetOp: OperationCatalogEntry, config: SecurityTestConfig): SequenceStep[] {
