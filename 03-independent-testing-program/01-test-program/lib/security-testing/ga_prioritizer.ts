...
function semanticRiskHint(gene: AttackGene): number {
    ...
    if (includesAny(resolver, ['secure', 'safe', 'sanitized', 'public', 'health', 'echo', 'time', 'feed'])) {
        return 0.15;
    }
    if (includesAny(resolver, ['admin', 'super', 'secret', 'private', 'internal', 'password', 'reset'])) {
        return 0.95;
    }
...
function scoreWithFsm(gene: AttackGene, seenResolvers: string[], seenTypes: string[], seenTemplates: string[]): number {
    return 0.35 * fsmProgress(gene)
        + 0.20 * securitySurfaceNovelty(gene, seenResolvers, seenTypes, seenTemplates)
        + 0.20 * executableScore(gene)
        + 0.10 * oracleSignalProxy(gene)
        + 0.15 * semanticRiskHint(gene);
}
