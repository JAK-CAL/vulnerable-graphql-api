...
type ObjectiveArchive = {[key: string]: ObjectiveArchiveEntry};
type PathCounts = {[key: string]: number};

const DefaultSensitiveFields = ['resetToken', 'internalNote', 'moderationNote'];
const ScalarSelectionFields = ['id', 'title', 'content', 'public', 'deleted', 'body', 'username', 'firstName', 'lastName', 'stdout', 'stderr', 'summary'];
const StatefulAuthorizationTypes = ['BOLA_READ', 'BOLA_UPDATE_DELETE', 'STALE_OBJECT_ACCESS'];
...
function sequenceUsesForeignReference(sequence: SequenceStep[], attacker: string): boolean {
    return sequence.some((step) => {
        return step.actor === attacker && variablesUsePool(step.variables);
    });
}
...
function sensitiveFieldNames(config: SecurityTestConfig): string[] {
    return unique(DefaultSensitiveFields.concat(config.hints.sensitiveFields || []));
}

function scalarSelection(fields: string[], config?: SecurityTestConfig): string {
    const sensitive = config ? sensitiveFieldNames(config) : DefaultSensitiveFields;
    const allowedNames = unique(ScalarSelectionFields.concat(sensitive));
...
function variablesUsePool(variables: any): boolean {
    return JSON.stringify(variables || {}).indexOf('${pool.') >= 0;
}

function sequenceUsesPool(sequence: SequenceStep[]): boolean {
    return sequence.some((step) => variablesUsePool(step.variables));
}

function matchesGeneObjectType(gene: AttackGene): (entry: OperationCatalogEntry) => boolean {
    return (entry) => !gene.objectType || entry.namedReturnType === gene.objectType;
}
...
function initializeArchive(chromosomes: SequenceChromosome[]): ObjectiveArchive {
...
function ensureObjective(archive: ObjectiveArchive, chromosome: SequenceChromosome): ObjectiveArchiveEntry {
    const key = chromosomeTargetKey(chromosome);
    if (!archive[key]) {
        archive[key] = makeArchiveEntry(chromosome);
    }
...
function isStatefulAuthorizationObjective(gene: AttackGene): boolean {
    return StatefulAuthorizationTypes.indexOf(gene.type) >= 0;
}

function statefulAuthorizationWeight(gene: AttackGene): number {
...
function initialSeedScore(chromosome: SequenceChromosome, seenResolvers: string[], seenTypes: string[], seenTemplates: string[]): number {
    const gene = chromosome.sourceGene;
    const dependencyReady = sequenceUsesPool(chromosome.sequence) ? 1 : 0.6;
    return 0.18 * statefulAuthorizationWeight(gene)
        + 0.22 * templateGoalWeight(gene)
        + 0.17 * dependencyReady
        + 0.20 * noveltyHint(chromosome, seenResolvers, seenTypes, seenTemplates)
        + 0.16 * semanticRiskHint(gene)
        + 0.07 * sequenceCostHint(chromosome);
}
...
function computeFeedback(chromosome: SequenceChromosome, results: any[], finding: Finding | undefined, beforePoolSize: number, pool: ObjectPool, seenResolvers: string[], seenTypes: string[], seenTemplates: string[]): RuntimeFeedback {
    ...
    return {
        ...
        dependencySatisfied: completed && sequenceUsesPool(chromosome.sequence),
...
function updateArchive(archive: ObjectiveArchive, chromosome: SequenceChromosome, finding: Finding | undefined, generation: number): void {
...
function pathRarityScore(pathCounts: PathCounts, chromosome: SequenceChromosome): number {
    const count = pathCounts[chromosomePathKey(chromosome)] || 0;
    return 1 / Math.sqrt(1 + count);
}

function activeObjectiveScore(chromosome: SequenceChromosome, archive: ObjectiveArchive, pathCounts: PathCounts, generation: number): number {
    const entry = ensureObjective(archive, chromosome);
    ...
    return 0.30 * uncovered
        + 0.20 * targetRarity
        + 0.15 * pathRarity
        + 0.10 * statefulAuthorizationWeight(chromosome.sourceGene)
        + 0.12 * templateGoalWeight(chromosome.sourceGene)
        + 0.08 * semanticRiskHint(chromosome.sourceGene)
        + 0.05 * cost
        + 0.05 * age
        + 0.10 * chromosome.fitness;
}

function selectExecutionBatch(chromosomes: SequenceChromosome[], archive: ObjectiveArchive, pathCounts: PathCounts, executedTargets: string[], generation: number, rng: () => number): SequenceChromosome[] {
    const candidates = chromosomes
    ...
    const batchSize = Math.min(candidates.length, 8);
    const selected: SequenceChromosome[] = [];
    const selectedKeys: string[] = [];
    const statefulQuota = Math.min(Math.ceil(batchSize * 0.45), candidates.filter((chromosome) => isStatefulAuthorizationObjective(chromosome.sourceGene)).length);

    function addFrom(pool: SequenceChromosome[], limit: number): void {
        for (let i = 0; i < pool.length && selected.length < limit; i++) {
            const key = chromosomeTargetKey(pool[i]);
    ...
    addFrom(candidates.filter((chromosome) => isStatefulAuthorizationObjective(chromosome.sourceGene)), statefulQuota);
    addFrom(candidates.filter((chromosome) => !isStatefulAuthorizationObjective(chromosome.sourceGene)), batchSize);
    addFrom(candidates, batchSize);
    return selected;
}
...
function tournament(population: SequenceChromosome[], archive: ObjectiveArchive, pathCounts: PathCounts, generation: number, rng: () => number): SequenceChromosome {
...
function repairGene(gene: AttackGene, catalog: OperationCatalogEntry[], config: SecurityTestConfig, id: string, generation: number, history: string[]): SequenceChromosome | undefined {
    ...
    if (gene.setupResolver && gene.targetResolver && !hasDependency(edges, gene.setupResolver, gene.targetResolver)) {
        const replacement = compatibleTargets(catalog, edges, gene.setupResolver, dependencyTagsFor(gene))
            .filter(matchesGeneObjectType(gene))[0];
        if (replacement) {
...
function mutateGene(parent: SequenceChromosome, catalog: OperationCatalogEntry[], config: SecurityTestConfig, rng: () => number, generation: number): SequenceChromosome {
    ...
    if (roll < 0.25 && gene.setupResolver) {
        ...
        const options = compatibleTargets(catalog, edges, gene.setupResolver, tags)
            .filter(matchesGeneObjectType(gene));
        if (options.length > 0) {
...
function nextGeneration(current: SequenceChromosome[], seeds: SequenceChromosome[], catalog: OperationCatalogEntry[], config: SecurityTestConfig, archive: ObjectiveArchive, pathCounts: PathCounts, rng: () => number, generation: number): SequenceChromosome[] {
