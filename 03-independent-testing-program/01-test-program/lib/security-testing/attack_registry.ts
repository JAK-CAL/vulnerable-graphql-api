...
function fieldNames(op: OperationCatalogEntry): string[] {
    return op.fields.map((field) => field.name);
}

function firstReadFor(createOp: OperationCatalogEntry, reads: OperationCatalogEntry[]): OperationCatalogEntry | undefined {
    return reads.filter((readOp) => sameReturnType(createOp, readOp))[0];
}

function pushBolaUpdateDeleteGene(
    genes: AttackGene[],
    createOp: OperationCatalogEntry,
    targetOp: OperationCatalogEntry,
    reads: OperationCatalogEntry[],
    owner: string,
    attacker: string,
    extraSensitiveFields: string[]
): void {
    if (!sameReturnType(createOp, targetOp)) {
        return;
    }
    ...
    gene.targetResolver = targetOp.name;
    const verifyRead = firstReadFor(createOp, reads);
    gene.verifyResolver = verifyRead ? verifyRead.name : undefined;
    gene.selectionSet = selectionForType(createOp.namedReturnType, fieldNames(targetOp), extraSensitiveFields);
...
export function buildAttackPopulation(catalog: OperationCatalogEntry[], owner: string, attacker: string, extraSensitiveFields: string[] = []): AttackGene[] {
    ...
    creates.forEach((createOp: OperationCatalogEntry) => {
        reads.forEach((readOp: OperationCatalogEntry) => {
            ...
            gene.selectionSet = selectionForType(createOp.namedReturnType, fieldNames(readOp), extraSensitiveFields);
    ...
    creates.forEach((createOp: OperationCatalogEntry) => {
        updates.forEach((updateOp: OperationCatalogEntry) => {
            pushBolaUpdateDeleteGene(genes, createOp, updateOp, reads, owner, attacker, extraSensitiveFields);
        });

        deletes.forEach((deleteOp: OperationCatalogEntry) => {
            pushBolaUpdateDeleteGene(genes, createOp, deleteOp, reads, owner, attacker, extraSensitiveFields);
        });

        deletes.forEach((deleteOp: OperationCatalogEntry) => {
            const readOp = firstReadFor(createOp, reads);
    ...
    admins.forEach((adminOp: OperationCatalogEntry) => {
        ...
        gene.selectionSet = scalarSelection(fieldNames(adminOp), extraSensitiveFields);
        gene.capabilities = ['LOW_PRIV_SESSION', 'ADMIN_LIKE_OP'];
        genes.push(gene);
    });

    sensitiveOps.forEach((op: OperationCatalogEntry) => {
        const fields = sensitiveFields(fieldNames(op), extraSensitiveFields);
        fields.forEach((fieldName: string) => {
            ...
            gene.selectionSet = scalarSelection(fieldNames(op), extraSensitiveFields);
