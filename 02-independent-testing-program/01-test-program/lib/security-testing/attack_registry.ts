...
function scalarSelection(fields: string[]): string {
    const allowed = fields.filter((field) => ['id', 'title', 'content', 'public', 'deleted', 'internalNote', 'body', 'moderationNote', 'username', 'firstName', 'lastName', 'resetToken', 'stdout', 'stderr'].indexOf(field) >= 0);
    if (allowed.length === 0) {
        return 'id';
    }
    return allowed.join(' ');
...
function selectionForType(typeName: string, fields: string[]): string {
    if (typeName === 'Post') {
    ...
    return scalarSelection(fields);
}

function sensitiveFields(fields: string[]): string[] {
    return fields.filter((field) => ['resetToken', 'internalNote', 'moderationNote'].indexOf(field) >= 0);
}

export function buildAttackPopulation(catalog: OperationCatalogEntry[], owner: string, attacker: string): AttackGene[] {
    ...
    creates.forEach((createOp: OperationCatalogEntry) => {
        reads.forEach((readOp: OperationCatalogEntry) => {
            ...
            gene.selectionSet = selectionForType(createOp.namedReturnType, readOp.fields.map((field) => field.name));
    ...
    creates.forEach((createOp: OperationCatalogEntry) => {
        updates.forEach((updateOp: OperationCatalogEntry) => {
            ...
            gene.selectionSet = selectionForType(createOp.namedReturnType, updateOp.fields.map((field) => field.name));
        ...
        deletes.forEach((deleteOp: OperationCatalogEntry) => {
            ...
            const verifyRead = reads.filter((readOp) => sameReturnType(createOp, readOp))[0];
            gene.verifyResolver = verifyRead ? verifyRead.name : undefined;
            gene.selectionSet = selectionForType(createOp.namedReturnType, deleteOp.fields.map((field) => field.name));
            gene.capabilities = ['AUTH(A)', 'AUTH(B)', 'OWN_OBJECT(A,T,id)', 'UPDATE_OR_DELETE_OP(T)', 'SIDE_EFFECT_CONFIRMATION'];
            genes.push(gene);
        });
    ...
    admins.forEach((adminOp: OperationCatalogEntry) => {
        ...
        gene.selectionSet = scalarSelection(adminOp.fields.map((field) => field.name));
        gene.capabilities = ['LOW_PRIV_SESSION', 'ADMIN_LIKE_OP'];
        genes.push(gene);
    });

    sensitiveOps.forEach((op: OperationCatalogEntry) => {
        const fields = sensitiveFields(op.fields.map((field) => field.name));
