...
export function defaultConfig(): SecurityTestConfig {
    ...
    return {
        ...
        outputDir: path.join(process.cwd(), '02-independent-testing-program/03-execution-results/security-results'),
        requestBudget: 20,
...
export function loadConfig(configPath?: string): SecurityTestConfig {
    ...
    const resolved = configPath || path.join(process.cwd(), '02-independent-testing-program/02-other-server-config/config.yaml');
