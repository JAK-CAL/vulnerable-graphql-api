...
function feedbackMarkdown(evaluations: EvaluationResult[], comparisons: GroundTruthComparison[]): string {
    ...
    lines.push('- `securePost` decoy resolver는 authorization check와 sensitive-field sanitization이 있는 secure ground-truth 항목이다. FP가 0이면 harness가 secure resolver를 취약점으로 과대보고하지 않았다는 신호다.');
...
function runReportMarkdown(evaluations: EvaluationResult[], comparisons: GroundTruthComparison[], groundTruth: GroundTruth): string {
    ...
    lines.push('- `securePost` decoy가 포함되어 FP를 함께 평가한다. FP가 0이면 secure resolver를 취약점으로 과대보고하지 않았다는 근거가 된다.');
