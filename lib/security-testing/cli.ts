...
async function runMode(mode: string, config: SecurityTestConfig, catalog: OperationCatalogEntry[], population: AttackGene[]): Promise<ModeRun> {
    ...
    for (let i = 0; i < prioritized.length; i++) {
        ...
        const sequence = lowerGeneToSequence(gene, catalog);
...
async function runSecurityRegression(config: SecurityTestConfig, options: RunOptions): Promise<void> {
    const catalog = loadOperationCatalog(config.operationCatalogPath);
...
async function main(): Promise<void> {
    ...
    console.log('Usage: npm run security:fuzz -- [catalog|run|eval] [--config config.yaml] [--catalog op_catalog.json] [--ground-truth ground_truth.json] [--profile course] [--method ours] [--seed 1] [--seeds 1,2,3] [--budgets 20,40,50,80] [--endpoint URL] [--out DIR] [--budget N]');
}
