...
export var SecureUpdateComment: GraphQLFieldConfig<any,any,any> = {
    type: CommentType,
    args: {
        id: {
            type: GraphQLID,
    ...
    resolve: async (_root: any, args: any, context: any) => {
        let comment = await db.Comment.findByPk(args.id);
        if (!comment || !context.user || String(comment.UserId) !== String(context.user.id) || comment.deleted === true) {
            return null;
        }
...
export var SecureDeleteComment: GraphQLFieldConfig<any,any,any> = {
    type: CommentType,
    args: {
        id: {
            type: GraphQLID,
