...
async function performReset(clearSessions: boolean): Promise<string> {
    ...
    await db.sequelize.query('DELETE FROM sqlite_sequence WHERE name IN ("Comments", "Posts", "Users")');
...
export async function resetServerState(clearSessions: boolean = false): Promise<string> {
    ...
    const run = resetQueue.then(async () => {
        message = await performReset(clearSessions);
    });
    resetQueue = run.then(() => undefined, () => undefined);
